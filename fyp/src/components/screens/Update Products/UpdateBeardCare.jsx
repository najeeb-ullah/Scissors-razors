...
const UpdateBeardCare = () => {
  ...
  const deleteBeardCare = (postid) => {
    fetch(`/deletebeardcare/${postid}`, {
      method: "delete",
      headers: {
        Authorization: "Bearer " + localStorage.getItem("jwt"),
      },
  ...
  const updateStock = (postTitle) => {
    ...
    if (!/^-?\d+\.?\d*$/.test(quantity)) {
      ...
      fetch(`/updatebeardstock/${postTitle}`, {
        method: "put",
        headers: {
              ...
              {data.map((item) => {
                return (
                  <tr key={item._id}>
                    ...
                    <td style={{ fontSize: "30px" }}>{item.price}</td>

                    <td>
                      <button
                        type="button"
                        className="btn btn-danger"
                        onClick={() => deleteBeardCare(item._id)}
                      >
...
export default UpdateBeardCare;
